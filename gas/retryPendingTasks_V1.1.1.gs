/**
 * retryPendingTasks_V1.1.1.gs
 * Enhanced fetch functionality with manual retry, better logging, and timeout handling
 * Changes from V1.1:
 * - Removed Column F (completed timestamp) check - only checks status column D
 * - Added proper handling for "no_results" status (marks as "no_results" instead of "fetched")
 * - Tasks with no SERP results are written as "Not Found" in ranking_results
 * - Tasks marked as "no_results" CAN be retried later (not skipped in retry function)
 * - Added cleanupFetchTriggers() function to remove duplicate triggers
 */
...
function retryPendingTasks() {
  ...
  for (let i = 0; i < logRows.length; i++) {
    ...
    // Only process tasks that aren't already complete
    if (!taskId) continue;
    if (status === 'fetched') continue; // Already fetched successfully
    // Note: "no_results" tasks CAN be retried (user may want to resubmit them later)
...
/**
 * Cleanup function to remove all fetch triggers
 * Use this when you have multiple triggers running and want to clean them up
 */
function cleanupFetchTriggers() {
  ...
  console.log(`🧹 Deleted ${existingTriggers.length} fetch triggers`);
  SpreadsheetApp.getUi().alert('🧹 Cleanup Complete', `Deleted ${existingTriggers.length} fetch triggers`, SpreadsheetApp.getUi().ButtonSet.OK);
  SpreadsheetApp.getActive().toast(`🧹 Deleted ${existingTriggers.length} fetch triggers`);
}
