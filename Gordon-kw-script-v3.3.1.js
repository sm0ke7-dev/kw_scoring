...
function setupRankingHeaders(sheet) {
  // v3.3.1 writes to a dedicated results sheet. This function now ensures
  ...
  rs.getRange(1, 4).setValue('Service/Niche'); // D (to be populated later)
...
function writeSingleRankingResult(sheet, row, result, targetRow) {
  // v3.3.1: Write to ranking_results with columns:
  // A: Position, B: URL, C: Keyword, D: Service/Niche (empty placeholder)
  const rs = getOrCreateRankingResultsSheet();
  ...
  const service = ''; // Empty placeholder for now (to be populated later)
  ...
  rs.getRange(writeRow, 1, 1, 4).setValues([[position, url, kw, service]]);
}

// v3.3.1 helpers for new sheets and skip logic
function getOrCreateRankingResultsSheet() {
  ...
  if (!rs) {
    ...
    rs.getRange(1, 4).setValue('Service/Niche');
    rs.getRange(1, 1, 1, 4).setFontWeight('bold');
  }
  return rs;
}
...
function submitScheduled() {
  ...
  for (let i = 0; i < rows; i++) {
    ...
    rs.getRange(sourceRow, 4).setValue(''); // Service/Niche (empty placeholder)
  }
