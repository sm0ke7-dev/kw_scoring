...
/**
 * Clear both config and ranking_results sheets (keeps row 1 headers)
 */
function clearResultsSheets() {
  ...
  if (configSheet) {
    ...
    if (lastRow >= 2) {
      configSheet.getRange(2, 1, lastRow - 1, configSheet.getLastColumn()).clearContent();
      cleared.push('config');
    }
  }
  ...
  if (cleared.length > 0) {
    ...
    SpreadsheetApp.getUi().alert('ℹ️ No Data', 'No data found to clear in config or ranking_results sheets.', SpreadsheetApp.getUi().ButtonSet.OK);
  }
}
...
function submitScheduled() {
  ...
  // Format Source Row column (D) as number to prevent auto-formatting as dates
  if (rows > 0) {
    cfg.getRange(startWriteRow, 4, rows, 1).setNumberFormat('0'); // Column D = Source Row
  }
...
function createFetchTrigger() {
  ...
  ScriptApp.newTrigger('fetchScheduled').timeBased().everyMinutes(5).create();
  SpreadsheetApp.getUi().alert('✅ Trigger Created', 'Fetch trigger set to run every 5 minutes.', SpreadsheetApp.getUi().ButtonSet.OK);
}
...
function runRankingCheckOnSheet() {
  SpreadsheetApp.getUi().alert('🚀 Scheduler Starting', 'Setting up submit and 5-min fetch schedulers now.', SpreadsheetApp.getUi().ButtonSet.OK);
  // Kick off first batch immediately
  submitScheduled();
  // Ensure periodic triggers are in place
  ensureSubmitTrigger();
  ensureFetchTrigger();
  SpreadsheetApp.getUi().alert('🚀 Scheduler Running', 'Submission and 5-min fetch schedulers are active. You can close the sheet; they will auto-stop when finished.', SpreadsheetApp.getUi().ButtonSet.OK);
  SpreadsheetApp.getActive().toast('🚀 Scheduler running: submit + 5-min fetch active');
  const sheet = SpreadsheetApp.getActive().getSheetByName('kw_variants') || SpreadsheetApp.getActiveSheet();
  sheet.getRange(1, 14).setValue('🚀 Scheduler running: submit + 5-min fetch active');
}