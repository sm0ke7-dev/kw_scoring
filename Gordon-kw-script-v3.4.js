...
function submitBatchRankingJobs(keywordsData) {
  try {
    ...
    if (responseCode !== 200) {
      console.error(`❌ API returned status code: ${responseCode}`);
      console.error('Response:', responseText);
      throw new Error(`DataForSEO API error: HTTP ${responseCode} - ${responseText.substring(0, 200)}`);
    }
...
function setupRankingHeaders(sheet) {
  // v3.4 writes to ranking_results with consolidated logs columns J-N
  // getOrCreateRankingResultsSheet() now handles all headers A-N
  const rs = getOrCreateRankingResultsSheet();
  // Headers are automatically set up by getOrCreateRankingResultsSheet()
}
...
function getOrCreateRankingResultsSheet() {
  ...
  // Ensure all headers exist (A-N)
  const headers = [
    'Ranking Position',      // A
    'Ranking URL',           // B
    'Keyword',               // C
    'rank_score',            // D (added by GRankingScore)
    'keyword_score',         // E (added by GRankingScore)
    'geo',                   // F (added by GRankingScore)
    'FINAL_SCORE',           // G (added by GRankingScore)
    'Ideal',                 // H (added by GRankingScore)
    'GAP',                   // I (added by GRankingScore)
    'Job_ID',                // J (consolidated from logs)
    'Source_Row',            // K (consolidated from logs)
    'status',                // L (consolidated from logs)
    'submitted_timestamp',   // M (consolidated from logs)
    'completed_timestamp'    // N (consolidated from logs)
  ];
  
  // Check if headers need to be set/updated
  const currentHeaders = rs.getRange(1, 1, 1, Math.max(rs.getLastColumn(), headers.length)).getValues()[0];
  ...
  if (needsUpdate || rs.getLastColumn() < headers.length) {
    rs.getRange(1, 1, 1, headers.length).setValues([headers]);
    rs.getRange(1, 1, 1, headers.length).setFontWeight('bold');
  }
...
/**
 * Clear ranking_results sheet (keeps row 1 headers)
 * Note: config sheet is not cleared as it contains configuration settings
 * v3.4: logs consolidated into ranking_results, so only one sheet to clear
 */
function clearResultsSheets() {
  const ss = SpreadsheetApp.getActive();
  let cleared = [];
  
  // Clear ranking_results sheet (includes consolidated logs data in columns J-N)
  ...
  if (cleared.length > 0) {
    SpreadsheetApp.getUi().alert('✅ Sheet Cleared', `Cleared data from: ${cleared.join(', ')} (headers preserved)`, SpreadsheetApp.getUi().ButtonSet.OK);
    SpreadsheetApp.getActive().toast(`✅ Cleared: ${cleared.join(', ')}`);
  } else {
    SpreadsheetApp.getUi().alert('ℹ️ No Data', 'No data found to clear in ranking_results sheet.', SpreadsheetApp.getUi().ButtonSet.OK);
  }
}
...
// ============================================================================
// LOGS FUNCTIONALITY (v3.4: consolidated into ranking_results columns J-N)
// ============================================================================
// NOTE: getLogsSheet() has been removed. All logs data now lives in
// ranking_results columns J-N (Job_ID, Source_Row, status, submitted_timestamp, completed_timestamp)
...
function ensureSubmitTrigger() {
  if (findTriggersByHandler('submitScheduled').length === 0) {
    ScriptApp.newTrigger('submitScheduled').timeBased().everyMinutes(10).create();
  }
}
...
function submitScheduled() {
  ...
  if (!taskIds || taskIds.length === 0) {
    const errorMsg = '❌ Submit failed: no task IDs. Check console logs for details.';
    SpreadsheetApp.getActive().toast(errorMsg);
    sheet.getRange(1, 14).setValue(errorMsg);
    console.error('Submit failed - check DataForSEO API key and response above');
    return;
  }

  // v3.4: Write logs data to ranking_results columns J-N (no longer using separate logs sheet)
  const rs = getOrCreateRankingResultsSheet();
  const now = new Date();
  const rows = taskIds.length;
  
  // Write "Pending" placeholders AND logs data to ranking_results at source row positions
  for (let i = 0; i < rows; i++) {
    ...
    rs.getRange(sourceRow, RANKING_COL.POSITION).setValue('Pending'); // A: Position
    rs.getRange(sourceRow, RANKING_COL.URL).setValue(''); // B: URL (empty)
    rs.getRange(sourceRow, RANKING_COL.KEYWORD).setValue(keyword); // C: Keyword
    
    // v3.4: Write logs tracking data to columns J-N
    rs.getRange(sourceRow, RANKING_COL.JOB_ID).setValue(taskIds[i]); // J: Job ID
    rs.getRange(sourceRow, RANKING_COL.SOURCE_ROW).setValue(sourceRow); // K: Source Row
    rs.getRange(sourceRow, RANKING_COL.STATUS).setValue('submitted'); // L: status
    rs.getRange(sourceRow, RANKING_COL.SUBMITTED_AT).setValue(now); // M: submitted timestamp
    rs.getRange(sourceRow, RANKING_COL.COMPLETED_AT).setValue(''); // N: completed timestamp (empty for now)
  }
  
  // Format Source Row column (K) as number to prevent auto-formatting as dates
  if (rows > 0) {
    // Find min and max source rows for batch formatting
    const sourceRows = toSubmit.map(t => t.row);
    const minSourceRow = Math.min(...sourceRows);
    const maxSourceRow = Math.max(...sourceRows);
    if (minSourceRow === maxSourceRow) {
      rs.getRange(minSourceRow, RANKING_COL.SOURCE_ROW).setNumberFormat('0');
    } else {
      // Format all rows in range (some might not have data, but that's okay)
      rs.getRange(minSourceRow, RANKING_COL.SOURCE_ROW, maxSourceRow - minSourceRow + 1, 1).setNumberFormat('0');
    }
  }
...
// ============================================================================
// FETCH PHASE (v3.4: reads logs data from ranking_results J-N, writes results to A-C)
// ============================================================================
...
function fetchScheduled() {
  ...
  // v3.4: Read logs data from columns J-N (Job_ID, Source_Row, status, submitted_timestamp, completed_timestamp)
  // Also read Keyword from column C for convenience
    ...
    if (res && res.rankings && res.rankings.length > 0) {
      ...
      if (targetRow && hasActualResultsAtRow(targetRow)) {
        ...
        if (existingKeyword === kw) {
          rs.getRange(i + 2, RANKING_COL.STATUS).setValue('fetched'); // L: status
      ...
      rs.getRange(i + 2, RANKING_COL.STATUS).setValue('pending'); // L: status - keep waiting
      pending++;
    }
  }
